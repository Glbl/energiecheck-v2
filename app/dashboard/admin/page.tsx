            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {employees.map((emp: any) => (
                <Link href={`/dashboard/admin/employee/${emp.id_employee}`} key={emp.id}>
                  <div className="flex items-center justify-between p-4 bg-black/40 rounded-3xl border border-white/5 hover:border-[#d4e137]/50 hover:bg-white/5 transition-all group cursor-pointer h-full">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white/10 group-hover:border-[#d4e137] transition-all">
                        <img 
                          src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/avatars/${emp.photo_url}`}
                          alt={emp.full_name}
                          className="w-full h-full object-cover"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${emp.full_name}&background=d4e137&color=black&bold=true`;
                          }}
                        />
                      </div>
                      <div>
                    ...
                    <div className="text-right flex items-center gap-2">
                      <div className="hidden group-hover:block animate-in fade-in slide-in-from-right-1">
                         <p className="text-[8px] text-[#d4e137] uppercase font-black tracking-tighter">Profil ansehen</p>
                      </div>
                      <ChevronRight className="text-gray-600 group-hover:text-[#d4e137] transition-colors" size={18} />
                    </div>
                  </div>
                </Link>
              ))}
