...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // SUSCRIPCIÓN REALTIME GRATUITA
    // Escucha cambios en clientes y logs para actualizar totales y funnel al instante
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'user_funnel_logs' },
        () => loadAdminData()
      )
      .subscribe();
  ...
  async function loadAdminData() {
    try {
      ...
      const { data: logs } = await supabase.from('user_funnel_logs').select('*').order('created_at', { ascending: false }).limit(10);
  ...
  if (loading) return <div className="min-h-screen bg-[#05070a] text-white flex items-center justify-center font-black italic">MASTER CONTROL LOADING...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-white font-sans text-left pb-20">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {funnelLogs.map((log) => (
              <div key={log.id} className="bg-black/40 border border-white/5 p-5 rounded-3xl">
                ...
                <p className="text-xs text-gray-500 uppercase font-bold tracking-tighter mb-1">Schritt:</p>
                <p className="text-sm font-black text-white italic uppercase truncate">{log.current_step}</p>
        ...
        {/* LISTA DE EMPLEADOS - CORRECCIÓN DE FOTO ESTRECHA */}
        <div className="bg-white/5 border border-white/10 rounded-[3rem] p-8 md:p-10 backdrop-blur-sm">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
            <h3 className="text-2xl font-black italic uppercase tracking-tight">Mitarbeiter Liste</h3>
            <div className="relative w-full md:w-72">
              <Search className="absolute left-4 top-3.5 text-gray-500" size={18} />
              <input type="text" placeholder="Suchen..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-sm outline-none focus:border-orange-500 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </div>
            ...
            {filteredEmployees.map((emp) => (
              ...
              >
                <div className="flex items-center gap-6 w-full min-w-0">
                  {/* FOTO CORREGIDA: Se añade flex-shrink-0 para que no se aplaste en móvil */}
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-500 to-orange-700 flex-shrink-0 flex items-center justify-center font-black text-xl italic text-black shadow-lg">
                ...
                <ChevronRight size={20} className="text-gray-700 group-hover:text-white flex-shrink-0" />
              </div>
            ))}
          </div>
        </div>
